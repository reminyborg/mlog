...
- **Editor commands are interactive-only:** bare `note` and `edit -t`/`edit -d` open `$EDITOR` and are for humans at a terminal. Agents must always pass note text (as args or on stdin) — with no text and no terminal, `note` errors out rather than hanging.
