...
- **Editor commands are interactive-only:** bare `note`, `note -e`, and `edit -t`/`edit -d` open `$EDITOR` and are for humans at a terminal. Agents must always pass note text (as args or on stdin); without a terminal these commands error out rather than hanging.
- **A day entry runs to the next H1.** `##` / `###` headings written inside a day are part of that day's notes, so `today`/`show` include them and `note` appends *after* them. Only `# ` starts a new section.
